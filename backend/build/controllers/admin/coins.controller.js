...
exports.getCoinSummary = getCoinSummary;
exports.getCoinHistory = getCoinHistory;
exports.adjustUserCoins = adjustUserCoins;
exports.resetUserCoins = resetUserCoins;
exports.getCoinExpiry = getCoinExpiry;
...
async function getCoinSummary(req, res) {
    try {
        const summaryRes = await (0, db_1.query)(`SELECT 
...
async function getCoinHistory(req, res) {
    try {
        const historyRes = await (0, db_1.query)(`SELECT u.id, u.email, u.coins, u.created_at
       FROM users u
...
async function adjustUserCoins(req, res) {
    try {
        ...
        const result = await (0, db_1.query)(`UPDATE users SET coins = coins + $1 WHERE id = $2 RETURNING coins`, [amount, userId]);
...
// Reset user coins
async function resetUserCoins(req, res) {
    try {
        const { userId } = req.body;
        if (!userId) {
            return res.status(400).json({ error: "userId is required" });
        }
        const result = await (0, db_1.query)(`UPDATE users SET coins = 0 WHERE id = $1 RETURNING coins`, [userId]);
        if (!result.rows.length) {
            return res.status(404).json({ error: "User not found" });
        }
...
async function getCoinExpiry(req, res) {
    try {
