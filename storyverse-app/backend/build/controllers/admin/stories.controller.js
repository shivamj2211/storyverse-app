...
async function getAllStories(req, res) {
    try {
        const storiesRes = await (0, db_1.query)(`SELECT s.id, s.slug, s.title, s.summary, s.cover_image_url, 
...
async function createStory(req, res) {
    try {
        ...
        const result = await (0, db_1.query)(`INSERT INTO stories (title, slug, summary, cover_image_url)
       VALUES ($1, $2, $3, $4)
       RETURNING id, slug, title, summary, cover_image_url`, [title, slug, summary || "", coverImageUrl || null]);
...
async function updateStory(req, res) {
    try {
        const { id } = req.params;
        const { title, slug, summary, coverImageUrl } = req.body;
        const result = await (0, db_1.query)(`UPDATE stories 
       SET title = COALESCE($1, title),
           slug = COALESCE($2, slug),
           summary = COALESCE($3, summary),
           cover_image_url = COALESCE($4, cover_image_url)
       WHERE id = $5
       RETURNING id, slug, title, summary, cover_image_url`, [title, slug, summary, coverImageUrl, id]);
...
async function deleteStory(req, res) {
    try {
        const { id } = req.params;
        const result = await (0, db_1.query)(`DELETE FROM stories WHERE id = $1 RETURNING id`, [id]);
        if (!result.rows.length) {
            return res.status(404).json({ error: "Story not found" });
        }
